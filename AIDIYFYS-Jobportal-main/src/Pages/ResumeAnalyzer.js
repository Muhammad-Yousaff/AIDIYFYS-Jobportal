...
const readFileAsArrayBuffer = (file) =>
  new Promise((resolve, reject) => {
    ...
    reader.onerror = () => {
      reject(new Error("Could not read binary file."));
    };
  ...
  const handleFileUpload = async (event) => {
    ...
    try {
      if (/\.pdf$/i.test(fileName) || file.type === "application/pdf") {
        ...
        if (!extracted) {
          setErrorMessage("No readable text found in this PDF. If it is scanned, paste text manually.");
          return;
        }
      ...
      if (/\.docx$/i.test(fileName)) {
        ...
        if (!extracted) {
          setErrorMessage("No readable text found in this DOCX file. Please paste text manually.");
          return;
        }
        setResumeText(extracted);
        return;
      ...
      if (/\.doc$/i.test(fileName)) {
        setErrorMessage("Legacy .doc files are not reliably readable in-browser. Please save as .docx or paste text manually.");
        return;
      }
      ...
      if (!canReadAsText) {
        setErrorMessage("Unsupported file. Upload PDF, DOCX, TXT, MD, or RTF, or paste resume text manually.");
        return;
      }
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="bg-white shadow-sm rounded-xl p-6 border border-slate-100">
              ...
              <label className="block text-sm font-medium text-slate-700 mb-2">
                Upload Resume (PDF/DOCX/TXT/MD/RTF)
              </label>
              <input
                type="file"
                accept=".pdf,.doc,.docx,.txt,.md,.rtf,.json,.csv,text/*,application/pdf,application/vnd.openxmlformats-officedocument.wordprocessingml.document,application/msword"
